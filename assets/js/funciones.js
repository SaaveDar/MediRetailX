document.addEventListener("DOMContentLoaded", function () {
    ...
    $("#buscarproducto").autocomplete({
        ...
        select: function (event, ui) {
            ...
            $("#ff").val(ui.item.ff);
            $('#prese').val(ui.item.prese);
            $('#cnc').val(ui.item.cnc);
            $("#cantidad").focus();
        }
    })

    $("#prosalida").autocomplete({
        minLength: 3,
        source: function (request, response) {
            $.ajax({
                url: "ajax.php",
        ...
        select: function (event, ui) {
            $("#ids").val(ui.item.ids);
            $("#codmeds").val(ui.item.codmeds);
            $("#codigobarras").val(ui.item.codigobarras);
            $("#prosalida").val(ui.item.label);
            $("#lotes").val(ui.item.lotes);
            $("#precios").val(ui.item.precios);
            $("#regsans").val(ui.item.regsans);
            $("#stocks").val(ui.item.stocks);
            $("#vencs").val(ui.item.vencs);
        }
    })
...
function editarProducto(id) {
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            $('#codmed').val(datos.codmed);
            $('#producto').val(datos.descripcion);
            $('#presentacion').val(datos.prese);
            $('#cnc').val(datos.cnc);
            $('#ff').val(datos.ff);
            $('#tipo').val(datos.tipo);
...
function editarvenci(id) {
    const action = "editarvenci";
    $.ajax({
        url: 'ajax.php',
        type: 'GET',
        async: true,
        ...
        success: function (response) {
            ...
            $('#codigobarra').val(datos.codigobarra);
            $('#codmed').val(datos.codmed);
            $('#producto').val(datos.med);
            $('#lote').val(datos.lote);
            $('#regsan').val(datos.codigoreg);
            $('#precio').val(datos.precio);
            $('#id').val(datos.idstock);
...
function nsalidapro(id) {
    const action = "nsalidapro";
    $.ajax({
        url: 'ajax.php',
        type: 'GET',
        async: true,
        ...
        success: function (response) {
            const datos = JSON.parse(response);
            $('#ids').val(datos.idstock);
            $('#codigobarras').val(datos.codigobarra);
            $('#codmeds').val(datos.codmed);
            $('#prosalida').val(datos.med);
            $('#lotes').val(datos.lote);
            $('#regsans').val(datos.codigoreg);
            $('#precios').val(datos.precio);
            $('#id').val(datos.idstock);
           // $('#tipo').val(datos.id_tipo);
           // $('#preses').val(datos.id_presentacion);
           // $('#laboratorio').val(datos.id_lab);
            $('#vencs').val(datos.vencimiento);
            $('#stocks').val(datos.stock);
            if (datos.vencimiento != '0000-00-00') {
                $("#accion").prop("checked", true);
            }else{
                $("#accion").prop("checked", false);
...
function editarRegsan(id) {
    const action = "editarRegsan";
    $.ajax({
        url: 'ajax.php',
        type: 'GET',
        async: true,
